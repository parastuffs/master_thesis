...
\ident Finally, the ground necessary to understand the inner working and challenges of some VPNs implementations will be covered with SSL/TLS and IPsec.
...
\begin{description}
	\item[Traffic protols] Encapsulating Security Payload (ESP,~\cite{rfc4303}) and Authentication Header (AH,~\cite{rfc4302}).
...
% ESP: TFC padding: traffic flow confidentiality, padding to hide the true nature of the packet, security mesure.
% Link to the figure fig:esp-packet-structure
Arguably, the features of AH can be offered by ESP.
...
IPsec can be deployed in two modes: transport for end-to-end service and protecting the payload, and tunnel to typically connect gateways and protect the entire IP packet.
