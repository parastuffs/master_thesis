...
Eventhough the MAC is not offloaded, the hardware implementation could have performed better.
...
\caption{IPsec: ping average response time -- software/Hardware}{\newline{}(a) Processing GCM in hardware yields at least as good results as CBC. Comparing the CBC mode without authentication in hardware with the GCM mode in software thus makes sensena yields 42\% less delay for larger payload size.\newline{}(b) With or without authentication, the hardware offers better results up to 13\% less delay.}
...
\end{tabular}
\caption{IPsec: ping average response time}{the latency is given in milliseconds. Even if there is no results for a hardware implementation of GCM, the result should have been close to those of \textit{ba411e-cbc256:none}.}
...
\noindent In hardware, as OpenVPN could not take effeciently advantage of the accelaration of the encryption, IPsec is even faster, ranging from 25\% to 33\% faster than OpenVPN.
...
As it was already the case for the ping benchmark, the overhead imposed by the manipulations of OpenVPN is extremely heavy: without encryption nor authentication, the CPU usage jumps from 7.16\% outide the tunnel to 42.60\% inside, and the throughput drops by 55\%, from 11.39MB/s to 5.18MB/s.

\noindent Encrypting the data only changes the throughput by 8\%, but the CPU usage is almost doubled.
Such a small decrease of performance is coherent with the results of the ping benchmark for the chunck of data size OpenVPN is supposed to process: around 1kB.
...
The softaware GCM results allow to open a discussion on this mode used in conjunction with IPsec.
...
\caption{Comparison of file transfer methods}{Out of the three methods, OpenSSH is to be prefered if the raw performance is the objective, but IPsec offers a higher effeciency. At the same time, OpenVPN is worse on all fronts. The background stripped bars are the CPU usage.}
...
OpenVPN sends one third of its packets at sizes that are highly ineffective, whilst IPsec manages a better frafmentation and limits the small packets to less than 10\% of the total.
